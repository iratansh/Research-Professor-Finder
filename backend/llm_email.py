...
#                 self.apiURL, headers=self.headers, json=data, timeout=timeout
#             )
...
class DeepSeekLLM:
    def __init__(self, apiKey):
        ...
        self.headers = {
            "Authorization": f"Bearer {self.apiKey}",
            "Content-Type": "application/json",
            "Referer": "https://localhost:5173/",  # Fixed header name
            "X-Title": "LocalDev",
        }

        self.session = requests.Session()
        retry_strategy = Retry(
            total=2,  # Reduced from default 3 attempts
            backoff_factor=0.3,  # Reduced backoff time
            status_forcelist=[429, 500, 502, 503, 504],
        )
        adapter = HTTPAdapter(max_retries=retry_strategy)
        self.session.mount("https://", adapter)

    def send_message(self, interests, name, timeout=15):  # Reduced default timeout
        data = {
            "model": "deepseek/deepseek-chat:free",
            "messages": [
                {
                    "role": "user",
                    "content": f"Draft an email to Professor {name} at University of Alberta. "
                    f"Focus on their work in: {interests}. "
                    f"Ask about research opportunities. "
                    f"Format: Email only, no extra text. "
                    f"Proper formal email structure required.",
                }
            ],
            "temperature": 0.7,
...
if __name__ == "__main__":
    llm = DeepSeekLLM(
        "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"
    )
